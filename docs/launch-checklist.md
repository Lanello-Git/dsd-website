...
- [ ] Put the GHL inbound webhook URL in `src/config/site.ts` as `formEndpoint` (the native form POSTs JSON), OR put the GHL form URL in `ghlFormEmbedUrl` to render the GHL iframe instead.
